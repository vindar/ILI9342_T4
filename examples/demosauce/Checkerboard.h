...
void Checkerboard::init(ILI9342Wrapper & tft) {
  _bgColor = color565( 0xff, 0xbb, 0xbb );
}
...
void Checkerboard::perFrame(ILI9342Wrapper & tft, FrameParams frameParams ) {
  ...
  for( float x=startX; x<w; x+=size ) {
    ...
    for( float y=startY; y<h; y+=size ) {
      ...
      uint_fast16_t color = color565( (sqX * sqY * 57), (isLight ? 0xff : 0), (isLight ? 0xff : 0) );
      // color ^= audioColor // flickers too much (and framerate is much lower !)
