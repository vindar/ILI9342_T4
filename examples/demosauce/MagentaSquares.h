...
void MagentaSquares::init(ILI9342Wrapper & tft) {
...
void MagentaSquares::perFrame(ILI9342Wrapper & tft, FrameParams frameParams ) {
  //uint_fast16_t w = tft.width();
  //uint_fast16_t h = tft.height();
  ...
  for( uint_fast8_t i=0; i<sqAcross; i++ ) {
    for( uint_fast8_t j=0; j<sqDown; j++ ) {
      ...
      if( (radius < _ripplePos) && (_ripplePos < radius+RIPPLE_STROKE) ) {
        clr = color565( 0xff, random(0x99,0xff), 0xff );

      } else {
        uint_fast8_t bright = (x+y+_step) & 0xff;
        clr = color565( bright, 0, bright );
      }
