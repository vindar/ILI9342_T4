...
void Cube3D::init(ILI9342Wrapper & tft) {
...
void Cube3D::perFrame(ILI9342Wrapper & tft, FrameParams frameParams ) {
  ...
  _phase += frameParams.timeMult * CUBE_3D_ROTATE_SPEED;
	...
	//uint_fast16_t border = color565( 0x08, 0, 0 );
	uint_fast16_t eraseColor = color565( 0, 0, 0 );
		...
		for( float y=-1.0f; y<=1.0f; y+=1.0f ) {
			for( float z=-1.0f; z<=1.0f; z+=1.0f ) {
				...
				if( pt.y >= 0 ) {	// sanity check
					tft.drawFilledCircle<true, false>( pt.x, pt.y , 7.0 - z3d, (eraseColor), 0);
				}
				...
				if( pt.y >= 0 ) {	// sanity check
					...
					uint_fast16_t fillColor = color565( 0x44, lerp8(0x66,0xff,bright), 0x44 );

					tft.drawFilledCircle<true, false>( pt.x, pt.y , 7.0 - z3d, (fillColor),0);
				}
