...
private:
  void _drawLine(ILI9342Wrapper & tft, float cosTilt, float sinTilt, float x, float y, float z, uint_fast16_t w_2, uint_fast16_t h_2, uint_fast16_t color );
...
void Sphere3D::init(ILI9342Wrapper & tft) {
  uint_fast16_t w = tft.width();
  uint_fast16_t h = tft.height();

  //_bgColor = color565( 0, 0x44, 0x44 ); // Only looks good in low light :P
...
void Sphere3D::_drawLine(ILI9342Wrapper & tft, float cosTilt, float sinTilt, float x, float y, float z, uint_fast16_t w_2, uint_fast16_t h_2, uint_fast16_t color ) {
  // Tilt!
  float tempY = y;
...
void Sphere3D::perFrame(ILI9342Wrapper & tft, FrameParams frameParams ) {
  ...
  if( _circStep > RADIATION_PX ) _circStep -= RADIATION_PX;
	...
	uint_fast16_t radiationColor = color565( 0, random((RADIATION_PX-_circStep)*(bright>>1)), random((RADIATION_PX-_circStep)*bright) ) | _bgColor;
	tft.drawFilledCircle<true, false>(w_2, h_2 , _baseCircSize + _circStep, (radiationColor), 0);
}
