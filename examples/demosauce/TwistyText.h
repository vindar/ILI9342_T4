...
void TwistyText::init(ILI9342Wrapper & tft) {
	_bgColor = color565( 0x0, 0x0, 0x33 );
}
...
void TwistyText::reset(ILI9342Wrapper & tft) {
...
void TwistyText::perFrame(ILI9342Wrapper & tft, FrameParams frameParams ) {
  ...
  uint_fast16_t h = (uint_fast16_t)tft.height();
	...
	for( uint_fast8_t c=0; c<(CHARS_PER_LINE*7); c++ ) {
		...
		uint_fast16_t sideColor = color565( 0xff * cosAngleAbs, 0x88 * cosAngleAbs, 0 );
	...
	for( int_fast8_t eqX=-1; eqX<=1; eqX+=2 ) {
		...
		for( int_fast8_t eqY=-1; eqY<=1; eqY+=2 ) {	// -1==bottom of screen, 1==top
			...
			for( uint_fast8_t row=0; row<3; row++ ) {	// 3 meter rows
				uint_fast16_t sideColor = color565( 0xaa + row*0x20, 0x55 + row*0x10, 0 );
				...
				if( newMeters[row] > prevMeter ) {
					...
					color = color565( 0x66, bright, bright );
