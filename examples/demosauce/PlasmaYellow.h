...
void PlasmaYellow::init(ILI9342Wrapper & tft) {
  _bgColor = color565( 0xff, 0xff, 0 );
}
...
void PlasmaYellow::perFrame(ILI9342Wrapper & tft, FrameParams frameParams ) {
  ...
  int_fast16_t h = (int_fast16_t)tft.height();
	...
	for( int_fast16_t x=0; x<w; x+=PLASMA_YELLOW_DITHER ) {
		for( int_fast16_t y=_ditherY; y<h; y+=PLASMA_YELLOW_DITHER ) {
			...
			uint_fast16_t color = color565( bright, bright, 0 );
