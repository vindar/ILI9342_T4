...
public:
  ...
  void init(ILI9342Wrapper & tft);
	void restart(ILI9342Wrapper & tft, uint_fast16_t color );
...
void TransitionHalftone::init(ILI9342Wrapper & tft) {

}

void TransitionHalftone::restart(ILI9342Wrapper & tft, uint_fast16_t inColor ) {
...
void TransitionHalftone::perFrame(ILI9342Wrapper & tft, FrameParams frameParams ) {
    ...
    for( int_fast8_t j=-down_2; j<=down_2; j++ ) {
      ...
      if( size < HALFTONE_CIRC_SIZE ) anySmallCircles = true;
			...
			if( size > prevSize ) {
				...
				tft.drawFilledCircle<false, true>( x, y  , size, _color, _color);
