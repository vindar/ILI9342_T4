...
void PlasmaCloud::init(ILI9342Wrapper & tft) {
	_bgColor = color565( 0x77, 0, 0xcc );
...
void PlasmaCloud::perFrame(ILI9342Wrapper & tft, FrameParams frameParams ) {
	...
	*/
	...
	for( uint_fast16_t x=0; x<w; x+=PLASMA_CLOUD_LINE_WIDTH ) {
		for( uint_fast16_t y=_ditherY; y<h; y+=PLASMA_CLOUD_STEP_Y ) {
			...
			uint_fast16_t color = color565( ((bright&0xfb)<<2), 0, bright );
