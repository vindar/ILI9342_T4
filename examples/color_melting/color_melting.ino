/********************************************************************
*
* ILI9342_T4 library example. vsync and screen tearing demo.
*
...
#define PIN_TOUCH_MISO 255   // optional  set this only if the touchscreen is connected on the same SPI bus
...
//#define PIN_TOUCH_MISO  1   // optional  set this only if the touchscreen is connected on the same SPI bus
//#define PIN_TOUCH_IRQ 32    // optional. set this only if the touchscreen is connected on the same SPI bus
//#define PIN_TOUCH_CS  31    // optional. set this only if the touchscreen is connected on the same spi bus
...
ILI9342_T4::ILI9342Driver tft(PIN_CS, PIN_DC, PIN_SCK, PIN_MOSI, PIN_RESET, PIN_TOUCH_MISO, PIN_TOUCH_CS, PIN_TOUCH_IRQ);


// 2 diff buffers with about 6K memory each
// (in this simple case, only 1K memory buffer would be enough). 
ILI9342_T4::DiffBuffStatic<6000> diff1;
ILI9342_T4::DiffBuffStatic<6000> diff2;
...
void setup()
    ...
    tft.setRotation(0);                 // start in landscape mode 320x240
    tft.setFramebuffer(internal_fb);    // set the internal framebuffer (enables double buffering)
    tft.setDiffBuffers(&diff1, &diff2); // set 2 diff buffers -> enables differential updates.
...
void loop()
    {
    clear(fb, ILI9342_T4_COLOR_WHITE); // draw a white background 
    ...
    drawDisk(fb, x, y, 60, c ? ILI9342_T4_COLOR_RED : ILI9342_T4_COLOR_BLUE); // alternate blue and red color at each frame. 
    tft.overlayFPS(fb); // draw the FPS counter on the top left, in red on a semi-transparent white background
    tft.overlayText(fb, "Color melting demo\nDrawing red / blue circle alternatively...", 2, 0, 14, ILI9342_T4_COLOR_BLACK, 1.0f, ILI9342_T4_COLOR_BLUE, 0.4f, 1);

    if (!can_do_vsync)
        {
        tft.overlayText(fb, "*** VSYNC DISABLED ***", 3, 0, 16, ILI9342_T4_COLOR_RED, 1.0f, ILI9342_T4_COLOR_BLACK, 0.5f);
        tft.overlayText(fb, "Connect the MISO pin to activate this feature...", 3, 2, 12, ILI9342_T4_COLOR_RED, 1.0f, ILI9342_T4_COLOR_BLACK, 0.5f);
        } else
        {
        if (tft.getVSyncSpacing() > 0)
            {
            tft.overlayText(fb, "VSync ON", 3, 0, 16, ILI9342_T4_COLOR_GREEN, 1.0f, ILI9342_T4_COLOR_BLACK, 0.5f);
            } else
            {
            tft.overlayText(fb, "VSync OFF", 3, 0, 16, ILI9342_T4_COLOR_RED, 1.0f, ILI9342_T4_COLOR_BLACK, 0.5f);
            }
        }
