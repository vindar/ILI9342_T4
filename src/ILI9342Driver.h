/******************************************************************************
*  ILI9342_T4 library for driving an ILI9342(C) screen via SPI with a Teensy 4/4.1
*  Implements vsync and differential updates from a memory framebuffer.
*
*  Copyright (c) 2023 Arvind Singh.  All right reserved.
*
...
/**
* THIS CODE IS ADAPTEE FROM THE ILI9341_T4 LIBRARY
**/

#ifndef _ILI9342_T4_ILI9342Driver_H_
#define _ILI9342_T4_ILI9342Driver_H_
...
// This libray uses specify hardware features of IMXRT1062 and will not work with another MCU...
    ...
    #error "Library ILI9342_T4 only supports Teensy 4/4.1/micromod."
#endif
...
#define ILI9342_T4_COLOR_BLACK 0x0
#define ILI9342_T4_COLOR_WHITE 0xffff
#define ILI9342_T4_COLOR_RED 0xf800
#define ILI9342_T4_COLOR_BLUE 0x1f
#define ILI9342_T4_COLOR_GREEN 0x7e0
#define ILI9342_T4_COLOR_PURPLE 0x8010
#define ILI9342_T4_COLOR_ORANGE 0xfc20
#define ILI9342_T4_COLOR_CYAN 0x7ff
#define ILI9342_T4_COLOR_LIME 0x7e0
#define ILI9342_T4_COLOR_SALMON 0xfc0e
#define ILI9342_T4_COLOR_MAROON 0x8000
#define ILI9342_T4_COLOR_YELLOW 0xffe0
#define ILI9342_T4_COLOR_MAJENTA 0xf81f
#define ILI9342_T4_COLOR_OLIVE 0x8400
#define ILI9342_T4_COLOR_TEAL 0x410
#define ILI9342_T4_COLOR_GRAY 0x8410
#define ILI9342_T4_COLOR_SILVER 0xc618
#define ILI9342_T4_COLOR_NAVY 0x10


    /** Configuration */

#define ILI9342_T4_DEFAULT_SPICLOCK 30000000         // default SPI write speed, some display can work up to 80Mhz...
#define ILI9342_T4_DEFAULT_SPICLOCK_READ 4000000     // default SPI read speed (much slower then write speed)

#define ILI9342_T4_DEFAULT_VSYNC_SPACING 0           // No VSync/Screen tearing protection by default.  
#define ILI9342_T4_DEFAULT_DIFF_GAP 6                // default gap for diffs (typ. between 4 and 50)
#define ILI9342_T4_DEFAULT_LATE_START_RATIO 0.3f     // default "proportion" of the frame admissible for late frame start when using vsync. 

#define ILI9342_T4_TRANSACTION_DURATION 3           // number of pixels that could be uploaded during a typical CASET/PASET/RAWR sequence. 
#define ILI9342_T4_RETRY_INIT 5                     // number of times we try initialization in begin() before returning an error. 
#define ILI9342_T4_TFTWIDTH 320                     // screen dimension x (in default orientation 0)
#define ILI9342_T4_TFTHEIGHT 240                    // screen dimension y (in default orientation 0)
#define ILI9342_T4_NB_SCANLINES ILI9342_T4_TFTHEIGHT// scanlines are mapped to the screen height
#define ILI9342_T4_MIN_WAIT_TIME  300               // minimum waiting time (in us) before drawing again when catching up with the scanline

#define ILI9342_T4_NB_PIXELS (ILI9342_T4_TFTWIDTH * ILI9342_T4_TFTHEIGHT)   // total number of pixels

#define ILI9342_T4_MAX_VSYNC_SPACING 5              // maximum number of screen refresh between frames (for sync clock stability). 
#define ILI9342_T4_DEFAULT_IRQ_PRIORITY 128         // default priority at which we run all irqs (dma, pit timer and spi interrupts).
#define ILI9342_T4_MAX_DELAY_MICROSECONDS 1000000   // maximum waiting time (1 second)

#define ILI9342_T4_TOUCH_Z_THRESHOLD     400        // for touch
#define ILI9342_T4_TOUCH_Z_THRESHOLD_INT 75         // same as https://github.com/PaulStoffregen/XPT2046_Touchscreen/blob/master/XPT2046_Touchscreen.cpp
#define ILI9342_T4_TOUCH_MSEC_THRESHOLD  3          //

#define ILI9342_T4_SELFDIAG_OK 0xC0                 // value returned by selfDiagStatus() if everything is OK.

#define ILI9342_T4_DEFAULT_FPS_COUNTER_COLOR_FG ILI9342_T4_COLOR_WHITE  // default values (color/opacity/position)
#define ILI9342_T4_DEFAULT_FPS_COUNTER_COLOR_BG ILI9342_T4_COLOR_BLUE   // for the FPS counter
#define ILI9342_T4_DEFAULT_FPS_COUNTER_OPACITY 0.5f                     // 
#define ILI9342_T4_DEFAULT_FPS_COUNTER_POSITION 0                       //

#define ILI9342_T4_ALWAYS_INLINE __attribute__((always_inline))
...
/*************************************************************************************************************
* ILI9342 / ILI9342C screen driver for Teensy 4/4.1.
*
...
*
* (3) Multiple buffering mode: no buffering / double buffering 
*
* (4) The object can also manage a XPT2046 touchscreen on the same SPI bus.
*
*
...
* 
* RGB565 layout as uint16_t:  bit 15            bit 0
*                                  RRRRR GGGGGG BBBBB
* 
*    -> red bits   [15-11] 
*    -> green bits [10-5]
...
public:
    ...
    *
    * - cs :  Pin connected to the CS pin on the display. Strongly recommended.
    ...
    *         
    * - sda:  Pin connected to the SDA/SDO (MOSI) pin on the display. Mandatory.   
    *         Must be a hardware MOSI pin for the SPI bus used on the Teensy.
    *         
    *         -> the ILI9342 display uses half-duplex communication hence does not
    *            need a SDI/MISO pin (but the touch screen does if present, see below). 
    *
    ...
    * 
    * - touch_miso: the MISO pin used (must be on the same spi bus as CSLK and SDA above)
    * 
    ...
    * 
    **/
    ILI9342Driver(uint8_t cs, uint8_t dc, uint8_t sclk, uint8_t sda, uint8_t rst = 255, uint8_t touch_miso = 255, uint8_t touch_cs = 255, uint8_t touch_irq = 255);
    ...
    **/
    bool begin(uint32_t spi_clock = ILI9342_T4_DEFAULT_SPICLOCK, uint32_t spi_clock_read = ILI9342_T4_DEFAULT_SPICLOCK_READ);


    /**
    * Query the value of the self-diagnostic register.
    * 
    * Should return ILI9342_T4_SELFDIAG_OK = 0xC0 if everything is fine.
    * 
    * Returns 0 if half_duplex is disabled. 
    **/
    int selfDiagStatus();
    ...
    **/
    void setSpiClock(int spi_clock = ILI9342_T4_DEFAULT_SPICLOCK);
    ...
    **/
    void setSpiClockRead(int spi_clock = ILI9342_T4_DEFAULT_SPICLOCK_READ);
    ...
    *
    * - the default value is ILI9342_T4_DEFAULT_IRQ_PRIORITY = 128.
    *
    ...
    **/
    void setIRQPriority(int irq_priority = ILI9342_T4_DEFAULT_IRQ_PRIORITY);
    ...
    **/
    ...
    /**
    * Set whether half duplex communication (master <-> slave) is perfoormed via the SDA line. 
    * 
    * If half-duplex is disabled, then the screen status cannot be queried and VSync is disabled. 
    * 
    * The default value is: halfDuplex(true)  [enabled]. 
    * 
    * Remark: this method should never be called except for debugging purposes...
    **/
    ...
    /**
    * Return whether half-duplex communication is enabled (true by default). 
    **/
    bool getHalfDuplex() const { return _halfduplex; }
    ...
    *    
    *    
    * Remark: contrarily to ILI9341, for ILI9342(C), the default orientation 0 is in landscape mode....
    * 
    ****************************************************************************************************
    ****************************************************************************************************/
    ...
    enum
        ...
        LANDSCAPE_320x240            = 0,
        PORTRAIT_240x230           = 1,
        LANDSCAPE_320x240_FLIPPED    = 2,
        PORTRAIT_240x230_FLIPPED   = 3,
        };
    ...
    * 
    * - orientation 0 and 2 in landscape mode : 320x240.
    * - orientation 1 and 3 in portrait mode : 240x320.
    *
    ...
    *  
    * with width =  320 if orientation is 0 or 2.
    *            =  240 if orientation is 1 or 3
    **/
    ...
    * Screen refresh rate. 
    * 
    * NOTE: Contrarily to ILI9341, the ILI9342(C) display as very few choices of refreshrate. 
    *       -> the refresh rate is always set at its max value (around 70Hz)
    *          the exact value can be queried with getRefreshRate().
    * 
    ****************************************************************************************************
    ****************************************************************************************************/
    ...
    **/
    // void  setRefreshMode(int mode); // NOT AVAILALBLE FOR ILI9342(C)
    ...
    *
    **/
    // int getRefreshMode() const { return _refreshmode; } // NOT AVAILALBLE FOR ILI9342(C)
    ...
    * Remark: calling this method resets the statistics.
    **/
    // void setRefreshRate(float refreshrate_hz); // NOT AVAILALBLE FOR ILI9342(C)
    ...
    * refresh mode set.
    * 
    * If half duplex is disabled, the real refresh rate cannot be queried so return the 
    * default value = 70.0Hz. 
    **/
    ...
    **/
    //  printRefreshMode();   // NOT AVAILALBLE FOR ILI9342(C)
    ...
    *
    * This number must be between -1 and 5 = ILI9342_T4_MAX_VSYNC_SPACING.
    *
    ...
    **/
    void setVSyncSpacing(int vsync_spacing = ILI9342_T4_DEFAULT_VSYNC_SPACING);
    ...
    * Remark: calling this method resets the statistics.
    **/
    void setLateStartRatio(float ratio = ILI9342_T4_DEFAULT_LATE_START_RATIO);
    ...
    * Remark: calling this method resets the statistics.
    **/
    void setDiffGap(int gap = ILI9342_T4_DEFAULT_DIFF_GAP);
    ...
    **/
    bool asyncUpdateActive() const { return (_dma_state != ILI9342_T4_DMA_IDLE); }
    ...
    **/
    void overlayText(uint16_t* fb, const char* text, int position, int line, int font_size,
                     uint16_t fg_color = ILI9342_T4_COLOR_WHITE, float fg_opacity = 1.0f,
                     uint16_t bg_color = ILI9342_T4_COLOR_BLACK, float bk_opacity = 0.0f,
    ...
    **/
    void overlayFPS(uint16_t* fb, 
                    int position = ILI9342_T4_DEFAULT_FPS_COUNTER_POSITION, 
                    uint16_t fg_color = ILI9342_T4_DEFAULT_FPS_COUNTER_COLOR_FG, 
                    uint16_t bg_color = ILI9342_T4_DEFAULT_FPS_COUNTER_COLOR_BG, 
                    float opacity = ILI9342_T4_DEFAULT_FPS_COUNTER_OPACITY);
    ...
    **/
    float statsRatioPixelPerFrame() const  { return (((float)_statsvar_uploaded_pixels.avg()) / ILI9342_T4_NB_PIXELS); }
    ...
    **/
    float statsDiffSpeedUp() const { return ((!diffUpdateActive())|| (_statsvar_transactions.count() == 0)) ? 1.0f : (((float)(ILI9342_T4_NB_PIXELS * 16)) / ((float)_spi_clock) * (1000000.0f / _statsvar_uploadtime.avg())); }
    ...
    **/
    void setTouchThreshold(int Zthreshold = ILI9342_T4_TOUCH_Z_THRESHOLD) { _touch_z_threshold = Zthreshold; }
...
private:
    ...
    ***********************************************************************************************************/
    ...
    using methodCB_t = void (ILI9342Driver::*)(void);   // typedef to method callback. 
    ...
    **/
    ...
    void _pushpixels(const uint16_t* fb, int x, int y, int len)  ILI9342_T4_ALWAYS_INLINE
        {
        switch (_rotation)
            {
    ...
    **/
    void _waitUpdateAsyncComplete()
        {
        if (_dma_state != ILI9342_T4_DMA_IDLE)
            {
            elapsedMillis em = 0;
            while ((_dma_state != ILI9342_T4_DMA_IDLE))
                {
                if (em > 2)
                    ...
                    if (em > 1000)
                        ...
                        _println("ILI9342_T4Driver: Hanging in _waitUpdateAsyncComplete()...");
                        em = 0;
                        }
                    }
                }
    ...
    ***********************************************************************************************************/
    ...
    static ILI9342Driver* volatile _dmaObject[3];  // points back to this-> (for the corresponding spi bus)

    enum
        {
        ILI9342_T4_DMA_IDLE = 0,
        ILI9342_T4_DMA_ON = 2,
        };
    ...
    void _spiInterrupt_software() ILI9342_T4_ALWAYS_INLINE
        {
    ...
    // called when doing partial diff redraw
    void _spiInterrupt_hardware() ILI9342_T4_ALWAYS_INLINE        
        {
     ...
     **/
    void _flush_cache(const void* ptr, size_t len) ILI9342_T4_ALWAYS_INLINE
        {
    ...
    * than ILI9342_T4_MAX_DELAY_MICROSECONDS
    **/
    void _delayMicro(uint32_t t)
        {
        if (t < ILI9342_T4_MAX_DELAY_MICROSECONDS) delayMicroseconds(t); // only allow delay of at most 0.1 second. 
        }
    ...
    **/
    int _nbScanlineDuring(uint32_t t) const
        {
        return ((t * ILI9342_T4_NB_SCANLINES) / _period);
        }
    ...
    **/
    uint32_t _timeForScanlines(uint32_t nbscanline) const
        {
        return (nbscanline * _period) / ILI9342_T4_NB_SCANLINES;
        }


    /**
    * Convert a ratio in [0.0f, 1.0f] to a scanline in [|0, ILI9342_T4_NB_SCANLINES - 1|]
    **/
    int _ratioToScanline(float r) const ILI9342_T4_ALWAYS_INLINE
        {
        int l = (int)(r * ILI9342_T4_NB_SCANLINES);
        l = ILI9342Driver::_clip((int)l, (int)0, (int)ILI9342_T4_NB_SCANLINES - 1);
        return l;
        }
    ...
    **/
    bool _isScanlineInRange(int start, int end)  ILI9342_T4_ALWAYS_INLINE
        {   
    ...
    **/
    uint32_t _microToExitRange(int start, int end)
        ...
        if ((delta < 0) || ((5 * delta) >= (4 * ILI9342_T4_NB_SCANLINES))) return 0; // invalid range (negative or too big). 
        int v = _getScanLine(false);
        if ((v < start) || (v > end)) return 0; // no wait needed. 
        return 1 + _microToReachScanLine(((end + 1) % ILI9342_T4_NB_SCANLINES), false); // make sur it is not 0. 
        }
    ...
    **/
    uint32_t _microToReachScanLine(int scanline, bool sync) ILI9342_T4_ALWAYS_INLINE
        {
        int now = _getScanLine(sync);
        const uint32_t diff = (now <= scanline) ? (uint32_t)(scanline - now) : (uint32_t)(scanline - now + ILI9342_T4_NB_SCANLINES);
        return (diff * _period) / ILI9342_T4_NB_SCANLINES;
        }
    ...
    ***********************************************************************************************************/
    ...
    static ILI9342Driver* volatile _pitObj[4];   // point back to this->
    ...
    void _setTimerIn(uint32_t us, methodCB_t timercb) ILI9342_T4_ALWAYS_INLINE
        {
        noInterrupts();
        _it.end(); // stop ongoing timer before changing callback method. 
        _pitcb = timercb;
        if ((us <= 3) || (us > ILI9342_T4_MAX_DELAY_MICROSECONDS)) { us = 3; } // asap
    ...
    void _setTimerAt(uint32_t ustime, methodCB_t timercb) ILI9342_T4_ALWAYS_INLINE
        {
    ...
    void _cancelTimer() ILI9342_T4_ALWAYS_INLINE
    {
        noInterrupts();
        _it.end();
        _istimer = false;
    ...
    bool _isTimer() const ILI9342_T4_ALWAYS_INLINE
    {
        return _istimer;
    }
    ...
    ***********************************************************************************************************/
    ...
    static uint16_t _blend32(uint32_t bg_col, uint32_t fg_col, uint32_t a) ILI9342_T4_ALWAYS_INLINE
        {
    ...
    static uint32_t _fetchbit(const uint8_t* p, uint32_t index) ILI9342_T4_ALWAYS_INLINE  { return (p[index >> 3] & (0x80 >> (index & 7))); }
    ...
    ***********************************************************************************************************/
    ...
    void _restartCpuTime() ILI9342_T4_ALWAYS_INLINE
    {
        _stats_elapsed_cputime = 0;
    }


    void _pauseCpuTime() ILI9342_T4_ALWAYS_INLINE
    {
        _stats_cputime += _stats_elapsed_cputime;
    }


    void _restartUploadTime() ILI9342_T4_ALWAYS_INLINE
    {
        _stats_elapsed_uploadtime = 0;
    }


    void _pauseUploadTime() ILI9342_T4_ALWAYS_INLINE
    {
    ...
    ***********************************************************************************************************/
    ...
    uint8_t _sclk, _mosi;                       // for the screen
    uint8_t _rst;                               // 

    bool _halfduplex;                           // true if half duplex communication is working 
    uint8_t _touch_miso, _touch_cs, _touch_irq; // pins for XPT2046 if present
    ...
    void _beginSPITransaction(uint32_t clock) ILI9342_T4_ALWAYS_INLINE
        {
    ...
    void _endSPITransaction() ILI9342_T4_ALWAYS_INLINE
        {
    ...
    uint8_t _readcommand8(uint8_t reg);


    void _writecommand_cont(uint8_t c) ILI9342_T4_ALWAYS_INLINE
        {
    ...
    void _writedata8_cont(uint8_t c) ILI9342_T4_ALWAYS_INLINE
        {
    ...
    void _writedata16_cont(uint16_t d) ILI9342_T4_ALWAYS_INLINE
        {
    ...
    void _writecommand_last(uint8_t c) ILI9342_T4_ALWAYS_INLINE
        {   
    ...
    void _writedata8_last(uint8_t c) ILI9342_T4_ALWAYS_INLINE
        {
    ...
    void _writedata16_last(uint16_t d) ILI9342_T4_ALWAYS_INLINE
        {
...
#define ILI9342_T4_TCR_MASK  (LPSPI_TCR_PCS(3) | LPSPI_TCR_FRAMESZ(31) | LPSPI_TCR_CONT | LPSPI_TCR_RXMSK | LPSPI_TCR_TXMSK)
#define ILI9342_T4_DC_PCS_MASK   (LPSPI_TCR_PCS(3))


    void _maybeUpdateTCR(uint32_t requested_tcr_state) ILI9342_T4_ALWAYS_INLINE
        {
        const uint32_t spi_tcr_new = (_spi_tcr_current & (~ILI9342_T4_TCR_MASK)) | (requested_tcr_state & ILI9342_T4_TCR_MASK);        
...
//                while ((_pimxrt_spi->FSR & 0x1f)); // wait for output fifo to be empty.
                ...
                _pimxrt_spi->TCR = (spi_tcr_new & (~ILI9342_T4_DC_PCS_MASK)) | (_tcr_dc_not_assert & ILI9342_T4_DC_PCS_MASK);
                if ( (spi_tcr_new & ILI9342_T4_DC_PCS_MASK) != (_spi_tcr_current & ILI9342_T4_DC_PCS_MASK) )
                    { // we must toogle DC
                    ...
                    if ( (spi_tcr_new & ILI9342_T4_DC_PCS_MASK) == (_tcr_dc_not_assert & ILI9342_T4_DC_PCS_MASK) )
    ...
    void _dma_assert_dc() ILI9342_T4_ALWAYS_INLINE
        {       
    ...
    void _dma_deassert_dc() ILI9342_T4_ALWAYS_INLINE
        {
    ...
    void _dma_set_base_tcr()
        ...
        if (_hardware_dc)
            {
            _dma_spi_tcr_assert = (_spi_tcr_current & ~ILI9342_T4_TCR_MASK) | (_tcr_dc_assert | LPSPI_TCR_FRAMESZ(7) | LPSPI_TCR_RXMSK);
            _dma_spi_tcr_deassert = (_spi_tcr_current & ~ILI9342_T4_TCR_MASK) | (_tcr_dc_not_assert | LPSPI_TCR_FRAMESZ(15) | LPSPI_TCR_RXMSK); // bug with | LPSPI_TCR_CONT
            }
        else
            {
            _dma_spi_tcr_assert = (_spi_tcr_current & ~ILI9342_T4_TCR_MASK) | (_tcr_dc_not_assert | LPSPI_TCR_FRAMESZ(7) | LPSPI_TCR_RXMSK);
            _dma_spi_tcr_deassert = (_spi_tcr_current & ~ILI9342_T4_TCR_MASK) | (_tcr_dc_not_assert | LPSPI_TCR_FRAMESZ(15) | LPSPI_TCR_RXMSK); // bug with | LPSPI_TCR_CONT
            }
        }
    ...
    void _directWriteLow(volatile uint32_t* base, uint32_t mask) ILI9342_T4_ALWAYS_INLINE { *(base + 34) = mask; }

    void _directWriteHigh(volatile uint32_t* base, uint32_t mask) ILI9342_T4_ALWAYS_INLINE { *(base + 33) = mask; }

    void _toggle(volatile uint32_t* base, uint32_t mask) ILI9342_T4_ALWAYS_INLINE { *(base + 35) = mask; }
    ...
    ***********************************************************************************************************/
    ...
    static ILI9342Driver* volatile _touchObjects[4];   // point back to this->
    ...
    int _mapTouchX(int x, int A, int B)
        {
        return ILI9342Driver::_clip<int>((int)roundf(ILI9342_T4_TFTWIDTH * ((float)(x - A)) / (B - A)), (int)0, (int)ILI9342_T4_TFTWIDTH - 1);
        }

    /** convert from raw value to y coord (in orientation 0) */
    int _mapTouchY(int y, int C, int D)
        {
        return ILI9342Driver::_clip<int>((int)roundf(ILI9342_T4_TFTHEIGHT * ((float)(y - C)) / (D - C)), (int)0, (int)ILI9342_T4_TFTHEIGHT - 1);
        }
